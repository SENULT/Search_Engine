...
class SearchEngine:
    ...
    def initialize_models(self):
        ...
        model_path = "../../../05_neural_models/deepct_convknrm_vi.pth"
        try:
            if os.path.exists(model_path):
                ...
                try:
                    self.convknrm.load_state_dict(checkpoint, strict=False)
                    self.deepct.load_state_dict(checkpoint, strict=False)
                    print("✅ Trained model loaded successfully")
                except Exception as e:
                    print(f"⚠️  Could not load full checkpoint: {e}")
                    print("⚠️  Using untrained models (random weights)")
            else:
    ...
    def search(self, query: str, method: str = "all", top_k: int = 10):
        ...
        # DeepCT + Conv-KNRM combined (single method)
        if method in ["neural", "deepct-convknrm", "all"]:
            results.extend(self._search_neural_combined(query_tokens, top_k))
    ...
    def _search_neural_combined(self, query_tokens, top_k):
        """
        DeepCT + Conv-KNRM Combined Method
        DeepCT computes term weights, Conv-KNRM does ranking with weighted terms
        """
        ...
        # Search more documents for better results
        num_docs_to_search = min(500, len(self.articles))
        
        print(f"🔍 Neural search: processing {num_docs_to_search} documents...")
        
        with torch.no_grad():
            for i in range(num_docs_to_search):
                ...
                try:
                    # Step 1: DeepCT computes semantic similarity score
                    deepct_score = self.deepct(query_tensor, doc_tensor).item()
                    
                    # Step 2: Conv-KNRM ranking score
                    convknrm_score = self.convknrm(query_tensor, doc_tensor).item()
                    
                    # Combine both scores: DeepCT provides term weighting, Conv-KNRM provides matching
                    score = (deepct_score * 0.5 + convknrm_score * 0.5)
                    
                    # Additional relevance boost from term overlap
                    query_set = set(query_indices[:10])
                    doc_set = set(doc_indices[:50])
                    overlap = len(query_set.intersection(doc_set))
                    if overlap > 0:
                        relevance_boost = overlap / max(len(query_set), 1) * 0.2
        ...
        print(f"✓ Top score: {scores[0][1]:.4f}, Lowest: {scores[-1][1]:.4f}")
        ...
        print(f"✓ Neural method found {len(results)} results")
...
async def startup_event():
    ...
    # Load data - Correct path from backend to data folder
    data_paths = [
...
class SearchRequest(BaseModel):
    query: str
    method: str = "all"  # "all", "bm25", "neural", "deepct-convknrm"
    top_k: int = 10
...
async def stats():
    return {
        ...
        "vocab_size": len(search_engine.vocab),
        "methods": ["BM25", "DeepCT+Conv-KNRM (Neural Combined)"]
    }
